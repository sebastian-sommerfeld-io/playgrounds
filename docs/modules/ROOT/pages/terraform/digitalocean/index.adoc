...
The xref:ROOT:terraform/digitalocean/step-8-https-config-broken.adoc[first attempt to set up HTTPS] did not use the ACME provider and did not work properly. So I turned to the link:https://registry.terraform.io/providers/vancluever/acme/latest/docs[Terraform ACME provider].
...
Currently, the major ACME CA is link:https://letsencrypt.org[Let's Encrypt], but the ACME support in Terraform can be configured to use any ACME CA, including an internal one that is set up using link:https://github.com/letsencrypt/boulder[Boulder]. For more detail on the ACME process, see link:https://letsencrypt.org/how-it-works[here]. For the ACME spec, click link:https://ietf-wg-acme.github.io/acme/draft-ietf-acme-acme.html[here].

Add the ACME provider to your `playground-provider.tf`:
[source, hcl-terraform]
----
terraform {
  required_providers {
...
Next add a `playground-cert.tf` and add the following contents:
[source, hcl-terraform]
----
...
Now you have to re-configure the load balancer so that ir accepts incoming HTTPS connections. So set `entry_port = 443` and `entry_protocol = "https"` plus link the certificate via the `certificate_name` option.
...
This is the actual graph from the configuration you set up by following this tutorial.
...
CAUTION: todo ... remove cert with doctl first ...
