...
rectangle digitalocean {
    ...
    'component cname as "playground" <<digitalocean_record>>
}
...
domain_host -right-> internet: http
internet -right-> domain: http
...
domain ~down~ url
'domain_host ~down~ ssl_cert
'cname -up-> domain

@enduml
----

NOTE: Communication takes place via HTTP. For HTTPS, you have to configure ssl at your domain providers management console and expose HTTPS services in your application.
